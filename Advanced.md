...
可以通过 `total_length` 设置文章截取字数，`ratio_string` 设置前中后截取比例。你可以降低字符数来让扣费变得更少，也可以增加字符数让摘要变得更准确。上限为5000，配置超过5000会重置为上限。
...
通过实践标明，1000字的文章截取，GPT就能很好地生成摘要。
...
**12、切换简介相关配置**  
5.5版本后，允许点击“切换”按钮切换简介，该功能默认开启。

**注意:** 切换简介本质上是经过一些处理后，重新生成一份简介，这将消耗key字数。

`summary_toggle` 控制是否开启切换简介功能，`summary_num` 切换时允许生成的简介总数，默认3个，切换简介将会在该个数内重复循环。
...
**12、摘要语音朗读**  
5.6版本后，允许点击"唱片"图标朗读生成好的摘要，`summary_speech` 控制是否启用该功能，默认开启，
